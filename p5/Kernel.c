	...
	behavior HoareCondition
			...
			method Signal (mutex: ptr to Mutex)
					var
						oldIntStat: int
						t: ptr to Thread
						mt: ptr to Thread              -- Ptr to thread holding the mutex which is the one who called signal

					if ! mutex.IsHeldByCurrentThread ()
						FatalError ("Attempt to signal a condition when mutex is not held")
					endIf
					oldIntStat = SetInterruptsTo (DISABLED)
					mt=mutex.heldBy                  -- mt points to thread holding the mutex
					t = waitingThreads.Remove ()

					if t
						t.status = READY
						mutex.heldBy = t
						readyList.AddToEnd (t)        -- Add the signalled thread to ready list .You can also add it to front
					endIf
					
					readyList.AddToEnd(mt)          -- Add the signalling thread after the signalled thread 
					mt.Sleep ()                     -- sleep the signalling thread
	...
	behavior ThreadManager
			...
			method Init ()
				...
				-- 
					...
					for a = 0 to MAX_NUMBER_OF_PROCESSES-1
						threadTable[a].Init ("thread - " + a)
						threadTable[a].status = UNUSED
						freeList.AddToEnd (&threadTable[a])     
					endFor
...
-----------------------------  ProcessManager  ---------------------------------
	...
	behavior ProcessManager
			...
			method Init ()
				...
				for b = 0 to MAX_NUMBER_OF_PROCESSES-1
					processTable[b].Init ()
					processTable[b].status = FREE
					freeList.AddToEnd (&processTable[b])
				endFor
...
-----------------------------  FrameManager  ---------------------------------
	...
	behavior FrameManager
			...
			method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
					-- 
					-- Allocates free frames and stores its physical address in AddrSpaces.
					-- It waits till frames needed are available. Once frames are available 
					-- updates the number of free frames available
					--
					...
					if waitList > 1
						restofThread.Wait(&frameManagerLock)
					endIf  
					...
					-- Identify which frames are available and update the entries in page table
					-- AddrSpace

					for i = 0 to numFramesNeeded-1
						frno = framesInUse.FindZeroAndSet ()
						fradd = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (frno * PAGE_SIZE)
						aPageTable.SetFrameAddr (i, fradd)
					endFor

					-- Adjust the number of free frames available
					numberFreeFrames = numberFreeFrames - numFramesNeeded
					aPageTable.numberOfPages = numFramesNeeded
					waitList = waitList - 1
					restofThread.Signal (&frameManagerLock)
			...
			method ReturnAllFrames (aPageTable: ptr to AddrSpace)
				--
				--  Looks through number of pages available and returns all of them
				--
				
				var 
					i, numFramesReturned, frameAddr, bitNumber: int

				-- Acquire the lock 
				frameManagerLock.Lock()
				numFramesReturned = aPageTable.numberOfPages
				
				-- For all the frames to be returned ,clear their bit
				...
				-- Update the number of free frames available
				numberFreeFrames = numberFreeFrames + aPageTable.numberOfPages

				-- Do Signal to wakeup thre first thread waiting for the frames
...
------------------------ StartUserProcess  -------------------------  
	...
	function StartUserProcess (arg: int)
	  ...
	  var 
			pcbp : ptr to ProcessControlBlock
			ofp: ptr to OpenFile
	  ...
	  pcbp = processManager.GetANewProcess ()
	  pcbp.myThread = currentThread
	  currentThread.myProcess = pcbp

	  addrSpace= &pcbp.addrSpace 
	  ofp = fileManager.Open ("TestProgram1")
	  ...
	  initPC = ofp.LoadExecutable(addrSpace)
	  if initPC < 0
			FatalError("Unrecoverable error.kernel process failed")
	  endIf

	  fileManager.Close(ofp)
	  ...
	  oldIntStat =  SetInterruptsTo (DISABLED)                               -- Step 1 : Disable Interrupts
	  addrSpace. SetToThisPageTable ()                                      -- Step 2 :  Init PTBR 
	  currentThread.isUserThread = true                                     -- Step 3 : Set isUserThread to true  
	  BecomeUserThread(initUserStackTop, initPC, initSystemStackTop)        -- Step 4 to Step 9
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
	...
	function Handle_Sys_Exec (filename: ptr to array of char) returns int
			...
			ofp = fileManager.Open (&strBuffer)	
			if ofp == null
				return -1
			endIf	
			
			initPC = ofp.LoadExecutable(&newAddrSpace)
			if initPC < 0
				return -1
			endIf
			...
			oldStatus =  SetInterruptsTo (DISABLED) 									-- Step 1 : Disable Interrupts
...
-----------------------------  Handle_Sys_Open  ---------------------------------
	...
	function Handle_Sys_Open (filename: ptr to array of char) returns int
			-- NOT IMPLEMENTED
			var  
					kerbuffer : array [MAX_STRING_SIZE] of char
					a: int
			
			print("Handle_Sys_Open invoked!")
			nl()
			a = currentThread.myProcess.addrSpace.GetStringFromVirtual(&kerbuffer, filename asInteger, MAX_STRING_SIZE)
			if a < 0
			 return -1
			endIf 
