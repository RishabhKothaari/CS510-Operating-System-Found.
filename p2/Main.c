...
-----------------------------  MoreThreadExamples  ---------------------------------
	...
	function MoreThreadExamples ()
			...
			-- Print this thread's name.  Note that we t~emporarily disable
...
-----------------------------  ProducerConsumer  ---------------------------------
	...
	var
		...
		myLock2: Mutex = new Mutex -- Mutex lock for Producer threads
		myLock3: Mutex = new Mutex -- Mutex lock for Consumer threads
		myLock4: Mutex = new Mutex -- Mutex lock for Print Buffer
	...
	function ProducerConsumer ()
			...
			myLock2.Init () -- Initialize Mutex lock  for Producer
			myLock3.Init () -- Initialize Mutex lock  for Consumer
			myLock4.Init () -- Initialize Mutex lock  for PrintBuffer
			full_buffs.Init(0)
...
-----------------------------  Dining Philosophers  ---------------------------------
	...
	class ForkMonitor
		...
		fields
			...
			conditionvar: array [5] of Condition -- For each philosopher ,condition variables is created
	...
	behavior ForkMonitor
		...
		method Init ()
			...
			conditionvar = new array of Condition{5 of new Condition}
		...
		method PickupForks (p: int)
			...
			-- It will put its status to HUNGRY and check if its left and
			-- right are eating or not. If they are eating the philosopher
			-- thread will have to wait using conditionvar.wait()

			monMutex.Lock()     
			status[p] = HUNGRY
			mon.PrintAllStatus()
			-- call test to see if my left neighbour and right neighbour are
			-- eating or not
			mon.testEatOrNot(p) 
			if(status[p] != EATING)
		...
		method PutDownForks (p: int)
			...
			-- It will put its statis to THINKING and make status of LEFT and
			-- right neighbour to EATING and wake up the waiting threads
			monMutex.Lock()
			status[p] = THINKING
			mon.PrintAllStatus()
			mon.testEatOrNot((p+1)%5)
			mon.testEatOrNot((p-1)%5)
			monMutex.Unlock()     
			endMethod
		...
		method testEatOrNot(p: int)
			-- This method is used to check if philosophers left & right neighbour are eating or not   
			-- If they are not eating ,then the philosopher should start eating but for
			-- that it will signal and wake up the first philosopher thread in Condition    

				if(status[p] == HUNGRY && status[(p+1)%5] != EATING && status[(p-1)%5] != EATING)
					status[p] = EATING
					mon.PrintAllStatus()
					conditionvar[p].Signal(&monMutex)
				endIf
