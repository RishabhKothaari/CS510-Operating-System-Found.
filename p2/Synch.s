...
_StringConst_5:
	.word	35			! length
	.ascii	"Thread does not have lock to Unlock"
	.align
_StringConst_4:
	.word	27			! length
	.ascii	"Thread already has the lock"
	.align
_StringConst_3:
	.word	51			! length
...
_CheckVersion_P_Synch_:
	...
	set	0x98ded8d1,r4		! myHashVal = -1730225967
	cmp	r3,r4
	be	_Label_9
	set	_CVMess1,r1
	call	_putString
...
! ASSIGNMENT STATEMENT...
	...
	mov	"\0\0AS",r10
!   NEW CLASS Constructor...
!   ZeroMemory: waitQueue = zeros  (sizeInBytes=12)
	load	[r14+8],r4
	add	r4,4,r4
	store	r0,[r4]
	store	r0,[r4+4]
	store	r0,[r4+8]
!   waitQueue = _P_List_List
	set	_P_List_List,r1
	load	[r14+8],r2
	store	r1,[r2+4]
! RETURN STATEMENT...
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (heldBy) then goto _Label_58
	load	[r14+8],r1
	load	[r1+16],r1
	cmp	r1,r0
	be	_Label_58
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if heldBy == _P_Thread_currentThread then goto _Label_68		(int)
	load	[r14+8],r1
	load	[r1+16],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (t) then goto _Label_71
	load	[r14+-32],r1
	cmp	r1,r0
	be	_Label_71
	jmp	_Label_72
