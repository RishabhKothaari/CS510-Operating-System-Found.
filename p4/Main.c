...
*/
      ...
      -- RunHoareSemanticsTests () -- code halfway implemented  
      RuntimeExit ()

    endFunction

-----------------------------  RunHoareSemanticsTests  ---------------------------------
-- This function tests the Hoare Semantics.  It creates a bunch of threads
-- (NUM_THREADS) and starts each thread running.  


  var 
    --mon: HMonitor
    TestTh: array [5] of Thread = new array of Thread {5 of new Thread }
    a: int = 0

  function RunHoareSemanticsTests ()
    
      TestTh[0].Init ("Thread-0")
      TestTh[0].Fork (resourceAllocation,2)

      TestTh[1].Init ("Thread-0")
      TestTh[1].Fork (resourceAllocation,4)

      TestTh[2].Init ("Thread-0")
      TestTh[2].Fork (resourceAllocation,3)

      TestTh[3].Init ("Thread-0")
      TestTh[3].Fork (resourceAllocation,8)

      TestTh[4].Init ("Thread-0")
      TestTh[4].Fork (resourceAllocation,1)
     
  endFunction


  function resourceAllocation (resourcereq: int)
    var 
      i: int
      w: int
    for i = 1 to 5
      --mon.Request (resourcereq)
      for w = 0 to 100
        currentThread.Yield ()
      endFor
      --mon.Return (resourcereq)
    endFor
  endFunction

--class HMonitor --unimplemented so commented
  --superclass Object
    --fields
      --conditionvar: array [5] of Condition -- For each thread ,condition variables is created
      --monMutex: Mutex                      -- Lock for Monitors shared data variables
   -- methods 
     -- Init ()
      --Request (resourceNeeded: int)
      --Return (resourceReturned: int)
  --endClass
