...
-------------------------Sleeping Barber------------------------------
	...
	-- A barber and set of customers are modeled using a barbcust array.Array index 0
	-- represents the barber and rest index represents customer.The barber and customer 
	-- can have status at any point in time and is stored in status array. A barber can
	-- have status as START and END. A customer can have status as BEGHR - Begin haircut
	-- SICHR - Sitting on chair,FINHR - Finish haircut, CLEAVE - Customer Leave.
	...
	function PrintBuffer (p1: int)
		...
		PrintLock.Lock()
		for q = 1 to waiting                     -- code for printing chairs starts
					print ("X")
		endFor

		for q = 1 to (CHAIRS-waiting)
					print ("-")
		endFor									 -- code for printing chairs ends
		
		print ("   ")							 -- print 3 spaces after printing chairs for alignment
			if p1 == 0							 -- code for printing status of barber/customer starts
				totalspaces = 0
			else
				totalspaces=(p1*5)+5
			endIf
			...
			switch status[p1]
				...
				case END:
					...
					break
			endSwitch
					nl ()						 -- code for printing status of barber/customer ends
...
-----------------------Gaming Parlor Problem------------------------------------
	...
	function GamingParlour ()
		-- Each individual group is modeled with a thread.Each group requests for the 
		-- number of dice needed before starting playing the game.The group plays for
		-- certain time and then returns the dice so that other players can get the 
		-- opportunity. Each group forks on playingGame method and passes an arguemnt as 
		-- the no of dice it wants to play. For playing the game each group requests the 
		-- front desk for the dice. The front desk is modeled using a Monitor with entry
		-- methods Request and Return. 
	...
	class FDeskMonitor
		superclass Object
		fields
			numberDiceAvail: int                 -- used to count dice available at any point in time
			monMutex: Mutex                      -- Lock for Monitors shared data variables
			aheadList: Condition				 -- aheadList used to have the first group wait 
			backList: Condition					 -- rest all other groups should wait on the remaining list 
			waitList: int                        
		methods 
			Init ()
